...
############## coturn base version     : turnserver-4.5.2.tar.gz             ########
...
echo "--------> this script installs coturn server version coturn-4.5.2 on aws ec2 instance..."
...
echo "--------> downloading turnserver v4.5.2 ..."
wget https://coturn.net/turnserver/v4.5.2/turnserver-4.5.2.tar.gz
tar -xf turnserver-4.5.2.tar.gz
sed -i '/FIPS_mode()/c if(EVP_default_properties_is_fips_enabled(NULL)){' turnserver-4.5.2/src/client/ns_turn_msg.c
cd turnserver-4.5.2
./configure

### compile & install coturn
make
echo "--------> installing turnserver v4.5.2 ..."
sudo make install
