...
// must be static
static rest_client_instance_t *get_rest_connection() {
...
int rest_client_make_query(rest_client_instance_t *inst, rest_response_buffer_t *container,int method, void *body ) {
    if (!inst->instance) {
...
static int rest_get_oauth_key(const uint8_t *kid, oauth_key_data_raw *key) {
    ...
    if (rest_client_make_query(rest,response_buffer,REST_METHOD_GET,(void *)kid) != 0 ) {
        return -1;
    };
...
static int rest_get_auth_secret(const uint8_t *usname, char *secret) {
    ...
    if (rest_client_make_query(rest,response_buffer,REST_METHOD_GET,(void *)usname) != 0 ) {
        return -1;
    };
