...
struct WindowsService {
  LPTSTR name; // Service name
  fnServiceStart start;
  fnServiceRun run;
  fnServiceStop stop;

  SERVICE_STATUS_HANDLE
  handle; // Current service status handle. don't need close it. See
          // https://learn.microsoft.com/windows/win32/api/winsvc/nf-winsvc-registerservicectrlhandlera?redirectedfrom=MSDN&devlangs=cpp&f1url=%3FappId%3DDev11IDEF1%26l%3DZH-CN%26k%3Dk(winsvc%252FRegisterServiceCtrlHandler)%3Bk(RegisterServiceCtrlHandler)%3Bk(DevLang-C%252B%252B)%3Bk(TargetOS-Windows)%26rd%3Dtrue
  HANDLE hEvent;
  SERVICE_STATUS status; // Current service status
};
WindowsService g_Service;
...
void ServiceLog(char *msg) {
  ...
  return; //*/
  std::ofstream ofs("d:\\coturn.log", std::ios_base::app);
  if (ofs.is_open()) {
...
int ServiceRun(char *name, fnServiceStart start, fnServiceRun run, fnServiceStop stop) {
  ...
  memset(&g_Service, 0, sizeof(WindowsService));
  g_Service.start = start;
  g_Service.run = run;
  g_Service.stop = stop;
  if (name) {
    size_t nLen = strlen(name);
    g_Service.name = new TCHAR[nLen + 1];
    if (g_Service.name)
  ...
  SERVICE_TABLE_ENTRY dispatchTable[] = {{g_Service.name, (LPSERVICE_MAIN_FUNCTION)ServiceMain}, {NULL, NULL}};
