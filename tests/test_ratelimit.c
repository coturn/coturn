...
static void test_occupancy_tracks_live_buckets(void) {
  ...
  ratelimit_init();
  TEST_ASSERT_EQUAL_UINT32(0u, ratelimit_count_occupied(60u));

  /* The first consume opens a window in exactly one bucket. */
  TEST_ASSERT_FALSE(ratelimit_consume_address(&a, 5u, 60u, NULL, NULL));
  TEST_ASSERT_EQUAL_UINT32(1u, ratelimit_count_occupied(60u));

  /* A zero window disables the scan (guards the exporter against a bad config). */
  TEST_ASSERT_EQUAL_UINT32(0u, ratelimit_count_occupied(0u));
}
...
static void test_collision_counter_counts_every_collision(void) {
  ...
  for (unsigned int i = 0; i < 65536 && !found_collision; ++i) {
    ...
    ratelimit_init();
    TEST_ASSERT_FALSE(ratelimit_consume_address(&victim, 1u, 60u, NULL, NULL));
    found_collision = ratelimit_consume_address(&collider, 1u, 60u, NULL, &first_collision) && first_collision;
  }
  ...
  ratelimit_consume_address(&collider, 1u, 60u, NULL, NULL);
